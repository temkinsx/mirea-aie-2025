...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Базовые «размерные» эвристики
    flags["too_few_rows"] = summary.n_rows < 100
    flags["too_many_columns"] = summary.n_cols > 100

    # Новые эвристики качества (HW03):
    # 1) Константные колонки (все непустые значения одинаковы)
    flags["has_constant_columns"] = any(
        (c.non_null > 0) and (c.unique <= 1) and (c.missing_share < 1.0)
        for c in summary.columns
    )
    # 2) Полностью пустые колонки (все значения = NaN)
    flags["has_all_missing_columns"] = any(c.non_null == 0 for c in summary.columns)
